...
## September 12 globe interaction

The opening globe now accepts mouse/pen dragging and focused keyboard rotation immediately. Its grab cursor and caption explain the interaction. Touch devices retain an explicit Rotate globe / Done rotating control so ordinary swipes can scroll the page. Returning from another projection or rebuilding the renderer resets the drag orientation along with the camera.

`npm run check` passes (137 tests, lint, asset checks, TypeScript, build, prerender). Browser checks verified direct dragging, arrow-key rotation, zoom, Escape, wheel scrolling over the map, Reset, and a small drag after switching back from Mercator. The 390px layout has no horizontal overflow and retains `touch-action: pan-y` by default. Physical touch gestures were not tested in the desktop browser. No browser errors or warnings appeared.
