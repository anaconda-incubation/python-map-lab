...
2. In nteract, run the generation cell in `notebook-checks/mobile-assets.ipynb` with this checkout as `root`. It calls `scripts/project-assets.py` and executes all 29 Python presets at both geometry qualities. Alternatively a configured NumPy environment can run `python scripts/project-assets.py`.
...
4. Run `npm run check` and `node scripts/compare-baseline.mjs ../app` against the accepted baseline. Review the map in the browser, especially Antarctica and moved cuts. Rerun the four changed lesson notebooks in nteract.
