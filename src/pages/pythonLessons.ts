...
def project(lon, lat):
    ...
    return x, y`,annotations:[{lines:[4,8],title:'01 · Shape the outline',body:'A1–A4 are the designers’ published polynomial coefficients, fitted to their chosen parallel spacing (see the explanation below). F is the polynomial on line 8. Try a small change to A1, such as 1.4, then run to see an Equal Earth-style variant.'},{lines:[9,11],title:'02 · Pair the function with its derivative',body:'Every coefficient appears in both F and its derivative. The matching horizontal compensation preserves area while the derivative remains nonzero.'}]}
,
