...
export function lessonNotebook(lesson: Lesson, code: string, geography: unknown) {
  const markdown = (id: string, source: string) => ({id,cell_type:'markdown',metadata:{},source:[source]})
  const cell = (id: string, source: string) => ({id,cell_type:'code',metadata:{},execution_count:null,outputs:[],source:[source]})
  return {
    nbformat:4,nbformat_minor:5,
    metadata:{kernelspec:{display_name:'Python 3',language:'python',name:'python3'},runt:{schema_version:'1',pixi:{channels:['conda-forge'],dependencies:['numpy','matplotlib','geopandas'],pypi_dependencies:[]}}},
    cells:[
      markdown('intro',`# ${lesson.name}: a world from a function\n\n${lesson.explanation}\n\nRun cells from top to bottom. This notebook includes Natural Earth land geometry and uses GeoPandas, NumPy and Matplotlib. nteract reads the declared dependencies; other notebook environments can use the setup cell. First-time installation requires internet access.`),
      cell('setup',`import importlib.util
import sys
...
print(f"Projected {len(coastlines):,} coastline segments.")`),
      cell('map',`fig, ax = plt.subplots(figsize=(12, 8), facecolor="#0B0D0C")
